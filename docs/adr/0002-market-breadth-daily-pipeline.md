...
- Use Massive adjusted daily aggregates for closes. `MASSIVE_API_KEY` remains a GitHub Actions secret and production requires confirmed public display rights.
- Run at 22:47 UTC and a deduplicated 23:32 UTC repair attempt on weekdays. Off-minute schedules reduce GitHub Actions schedule congestion; the second run is the repair path.
