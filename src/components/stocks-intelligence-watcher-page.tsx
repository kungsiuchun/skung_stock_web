...
import {
  ...
  STOCKS_WATCHER_PRICE_RANGES,
} from "@/lib/stocks-intelligence-watcher-session";
import type { StocksWatcherPriceRange } from "@/lib/stocks-intelligence-watcher-session";
import {
...
interface SharedPriceAxisRange {
  min: number;
  max: number;
}
...
const formatOptionalSignedExposure = (value: number | null | undefined) => {
...
const priceRangeLabel: Record<StocksWatcherPriceRange, string> = {
  "1mo": "1M",
  "3mo": "3M",
  "1y": "1Y",
};
...
const effectiveLegOpenInterest = (leg: RawOptionLeg | undefined, explicit?: unknown) => {
  if (explicit !== undefined && explicit !== null) return Math.max(0, optionLegNumber(explicit));
  const openInterest = optionLegNumber(leg?.openInterest);
  return Math.max(0, openInterest);
};
...
const buildStrikeRowsFromOptionRaw = (
  ...
  if (!chain) return [];
  const spot = optionLegNumber(chain.spot);
  ...
  return strikes.map((strike) => {
    ...
    const callGex = optionLegNumber(exposure?.callGex);
    const putGex = optionLegNumber(exposure?.putGex);
    return {
      strike,
      callOpenInterest,
      putOpenInterest,
...
const buildExpiryRowsForSelector = (snapshot: StocksWatcherSnapshot | null): ExpirySelectorRow[] => {
  if (!snapshot) return [];
  ...
  return expiries.map((expiry) => {
    ...
    return {
      ...
      dominantType,
      netGex: rows.length > 0 && rows.every((row) => typeof row.netGex === "number" && Number.isFinite(row.netGex))
        ? rows.reduce((sum, row) => sum + (row.netGex || 0), 0)
        : undefined,
      netDex: rows.length > 0 && rows.every((row) => typeof row.netDex === "number" && Number.isFinite(row.netDex))
        ? rows.reduce((sum, row) => sum + (row.netDex || 0), 0)
        : undefined,
    };
  });
};
...
const summaryFromLoadedOptionChain = (chain: RawOptionChain | null): ExpirySelectorRow | null => {
  ...
  const legs = [
    ...
    ...(chain.puts || []).map((leg) => ({ ...leg, type: "P" as const })),
  ];
  if (legs.length === 0) return null;
  const total = (key: "openInterest" | "volume", type?: "C" | "P") => legs
    .filter((leg) => !type || leg.type === type)
    .reduce((sum, leg) => sum + optionLegNumber(leg[key]), 0);
  ...
  return {
    ...
    loaded: true,
    openInterest: total("openInterest"),
...
const toChartRows = (points: RawHistoryPoint[]): PriceChartRow[] => {
  let previousTime = 0;
  return points
    .map((point, index) => {
...
const getSharedPriceAxisRange = (
  result: NativeToolResult,
  gexRows: ChartGexAggregateRow[],
): SharedPriceAxisRange | null => {
  const priceRows = toChartRows(historyFromResult(result));
  const values = [
    ...priceRows.flatMap((row) => [row.low, row.high].filter((value): value is number => typeof value === "number")),
    ...gexRows.map((row) => row.strike),
  ].filter((value): value is number => typeof value === "number" && Number.isFinite(value));
  if (values.length === 0) return null;
  const min = Math.min(...values);
  const max = Math.max(...values);
  const padding = Math.max((max - min) * 0.04, 0.01);
  return { min: min - padding, max: max + padding };
};

const OhlcVolumeChart = ({
  result,
  priceRange = "3mo",
  onPriceRangeChange = () => undefined,
  sharedPriceAxisRange,
  sharedAxisStrikes = [],
  onPriceAxisCoordinatesChange,
}: {
  result: NativeToolResult;
  ...
  const points = useMemo(() => toChartRows(allPoints), [allPoints]);
  ...
  const sharedAxisStrikeKey = sharedAxisStrikes.join("|");
  const axisStrikes = useMemo(
    () => sharedAxisStrikes.filter((strike) => Number.isFinite(strike)),
    [sharedAxisStrikeKey],
  );
  ...
  useEffect(() => {
    ...
    if (sharedPriceAxisRange) {
      priceSeries.applyOptions({
        autoscaleInfoProvider: () => ({
          priceRange: { minValue: sharedPriceAxisRange.min, maxValue: sharedPriceAxisRange.max },
        }),
      });
    }
      ...
      const coordinates = Object.fromEntries(
        axisStrikes.flatMap((strike) => {
          ...
          return coordinate === null || !Number.isFinite(coordinate)
            ? []
            : [[String(strike), chartTop + coordinate]];
        }),
      );
      onPriceAxisCoordinatesChange(coordinates);
    };
    const scheduleAxisCoordinates = () => {
  ...
  }, [axisStrikes, chartStyle, onPriceAxisCoordinatesChange, points, sharedPriceAxisRange?.min, sharedPriceAxisRange?.max]);

  if (points.length < 2) return <ToolTextSummary result={result} />;

  return (
    <div
      className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-slate-800 bg-[#05080d]"
      data-chart-style={chartStyle}
      data-price-chart-range={priceRange}
      data-shared-price-axis-range={sharedPriceAxisRange ? `${sharedPriceAxisRange.min}:${sharedPriceAxisRange.max}` : undefined}
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {STOCKS_WATCHER_PRICE_RANGES.map((range) => (
            <button
              key={range}
              type="button"
      ...
      <div className="flex items-center justify-between border-t border-slate-800 px-3 py-2 text-xs text-slate-500">
        ...
        <span>{priceRangeLabel[priceRange]} Daily | {points.length} bars | Yahoo</span>
      </div>
    </div>
  );
};
...
export function StocksIntelligenceWatcherPage({ onBackToWork }: StocksIntelligenceWatcherPageProps) {
  ...
  const selectWatchlistSymbol = (symbol: string) => {
    ...
    if (nextSymbol !== selectedSymbol) {
      // Change the selected row immediately and clear the previous ticker's detail.
      // The request below remains abortable, so an old response cannot repaint it.
  ...
  const updatePriceAxisCoordinates = useCallback((coordinates: Record<string, number> | null) => {
    setPriceAxisCoordinates((current) => {
      ...
      const nextEntries = Object.entries(coordinates);
      if (
        nextEntries.length === Object.keys(current).length
        && nextEntries.every(([strike, coordinate]) => Math.abs((current[strike] ?? Number.NaN) - coordinate) < 1)
      ) {
        return current;
      }
      return coordinates;
  ...
  useEffect(() => {
    if ((activeTab === "Options" && activeSubTab === "Overview") || activeTab === "Chart") {
    ...
    return (
      <section className="siw-panel siw-primary-panel siw-chart-panel" data-primary-tab-panel="Chart">
        ...
        {tabPanelState.loading && (
          <div className="space-y-3"><SkeletonBlock className="h-10 w-56" /><SkeletonBlock className="h-[34rem] w-full" /></div>
        )}
        {tabPanelState.error && <ErrorBanner message={tabPanelState.error} onRetry={() => void loadTopTab("Chart", true)} />}
        {!tabPanelState.loading && !tabPanelState.error && !priceResult && (
          ...
          <div className="siw-chart-options-layout">
            <div ref={pricePanelRef} className="siw-chart-price-panel">
              <OhlcVolumeChart
                ...
                sharedPriceAxisRange={sharedPriceAxisRange}
                sharedAxisStrikes={gexRows.map((row) => row.strike)}
            ...
            <aside ref={gexPanelRef} className={`siw-chart-gex-panel ${hasSharedGexAxis ? "is-axis-aligned" : ""}`} data-chart-gex-by-strike>
              ...
              {hasChartGex && (
                <div
                  ...
                  style={hasSharedGexAxis ? { top: 0, bottom: 0 } : undefined}
                >
                  {gexRows.map((row) => {
  ...
  const renderOptionsOverview = () => (
    <div className="flex min-h-0 flex-1 flex-col p-3 sm:p-4">
      ...
      {!expiryOverviewState.loading && (!modeAvailable || chartRows.length === 0) && (
  ...
  const renderStatsReferencePanel = () => {
    ...
    const yahooQuote = rawRecord(statsRaw.quote) || {};
    const yahooSummary = rawRecord(statsRaw.summary) || {};
    const yahooProfile = rawRecord(yahooSummary.assetProfile) || {};
    const yahooStats = rawRecord(yahooSummary.defaultKeyStatistics) || {};
    const yahooFinancial = rawRecord(yahooSummary.financialData) || {};
    const yahooDetail = rawRecord(yahooSummary.summaryDetail) || {};
    const fromRaw = (keys: string[]) => {
      for (const key of keys) {
        for (const source of [statsRaw, betaRaw, yahooQuote, yahooProfile, yahooStats, yahooFinancial, yahooDetail]) {
          const value = source[key];
          const nestedValue = rawRecord(value)?.raw ?? value;
    ...
    const rows = [
      ["Exchange", String(fromRaw(["exchange", "fullExchangeName"]) || "n/a")],
      ["Market State", String(fromRaw(["marketState"]) || quote?.marketState || "n/a")],
      ["Sector", sector],
      ["Industry", industry],
    ...
    <section className="siw-app min-h-dvh w-full overflow-visible text-slate-100 lg:h-full lg:min-h-full lg:overflow-hidden" data-stocks-watcher-root>
      <div className={`siw-replica-shell ${watchlistCollapsed ? "is-rail-collapsed" : ""}`} data-watcher-replica>
        ...
        <main ref={detailPanelRef} className="siw-main">
          <header className={`siw-hero ${loading ? "is-loading" : ""}`} data-selected-symbol={selectedSymbol}>
          ...
          <div className="siw-main-scroll">
            {error && <ErrorBanner message={error} onRetry={refreshCurrent} />}
            {loading && loadingSymbol === selectedSymbol && (
              <section className="siw-ticker-transition" data-ticker-loading aria-live="polite">
                <div>
                  <span>Loading {selectedSymbol} from Yahoo</span>
                  <em>Previous ticker data is intentionally hidden.</em>
                </div>
                <div className="siw-ticker-transition-skeletons" aria-hidden="true">
