          ...
          <div className={standalone ? "flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between" : "siw-spx-market-breadth-heading"}>
            <div>
              ...
              <p className={standalone ? "mt-3 max-w-3xl text-xs leading-5 text-zinc-500" : "siw-spx-market-breadth-subtitle"}>
                Daily participation, sector leadership, and long-term trend strength. EOD snapshot publishes after the provider's next-day window, not intraday.
              </p>
            </div>
          ...
          {data && (
            <div className="mt-5">
              ...
              <p className="border-x border-b border-zinc-800 bg-[#111416] px-3 py-2 text-[0.6rem] leading-4 text-zinc-500">
                EOD as of {formatDate(data.priceAsOf)}. This control reloads the published snapshot; the scheduled market-data job waits for the provider's next-day EOD publication window. Sources: {data.sources.map((source) => `${source.provider} — ${source.role}`).join(" · ")}
              </p>
            </div>
          )}
        </header>
