...
- Council and CIO both use `openai/gpt-5-mini` with strict JSON Schema, no `temperature`, and `reasoning.effort="minimal"`. Every structured GPT-5 Mini request is Azure-only: send `order` and `only` as `azure`, set `allow_fallbacks=false`, keep `require_parameters=true`, and retry at most once against the same Azure provider. Accept `azure` endpoint variants only; every other resolved provider is an `UNAPPROVED_PROVIDER` fail-closed contract violation. An HTTP 404 is `PROVIDER_UNAVAILABLE`, never timeout/schema evidence. Council uses `max_completion_tokens=1024`, a 45-second attempt timeout, a shared 100-second deadline, and an 8KB role-projection hard limit. CIO uses `max_completion_tokens=1536` and its existing timeout profile. There is no cross-model or cross-provider fallback.
