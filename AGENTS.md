...
- Council and CIO both use `openai/gpt-5-mini` with strict JSON Schema, no `temperature`, and `reasoning.effort="minimal"`. Every structured GPT-5 Mini request is OpenAI-only: send `order` and `only` as `openai`, set `allow_fallbacks=false`, keep `require_parameters=true`, and retry at most once against the same OpenAI provider. Any resolved provider outside OpenAI is an `UNAPPROVED_PROVIDER` fail-closed contract violation. Council uses `max_completion_tokens=1024`, a 45-second attempt timeout, a shared 100-second deadline, and an 8KB role-projection hard limit. CIO uses `max_completion_tokens=1536` and its existing timeout profile. There is no cross-model or cross-provider fallback.
...
- Off-hours manual decision triggers use the fixed 2026-07-13 14:45 ET `UAT_REPLAY`, clearly marked non-live, and make no model call. The authenticated `?uat_llm` route first runs one GPT-5 Mini compatibility probe, then runs the same fixture through four live Council calls and one CIO call without changing position or signal journals; Telegram requires explicit `?deliver` and starts with `SYSTEM UAT｜非即時訊號｜不可交易`.
