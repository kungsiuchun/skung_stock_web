...
- Telegram starts with final Risk-Gated action, snapshot SPX price, and scheduled ET time. It renders QM/CM/NT/PA in that order with each agent's validity, confidence, reasoning, and attempt status; an invalid agent is counted as `INVALID`, never as a real HOLD vote.
- Council agents use `google/gemma-4-26b-a4b-it` with strict JSON Schema, `temperature=0`, a compact v2 wire contract, and `max_tokens=512` (`640` only for one same-model retry after truncation). They output `CALL`, `PUT`, or `HOLD`, never `OPEN_*`. Requests may only use the allowlist `DeepInfra -> Parasail`: send this same ordered pair in both `order` and `only`, retain `allow_fallbacks=true` only within that pair, and rotate retry attempt two to Parasail first while ignoring a provider named by failure metadata. Any resolved provider outside the allowlist is a fail-closed contract violation. Keep `require_parameters=true`, a 45-second attempt timeout, a shared 100-second Council deadline, and an 8KB role-projection hard limit. CIO remains `openai/gpt-5-mini` with strict JSON Schema, `max_completion_tokens=520`, no `temperature`, and its existing timeout profile. There is no cross-model fallback.
- Valid AI confidence is 1-100; zero is reserved for pipeline-generated invalid/degraded results. Every Council/CIO claim, including HOLD conflicts, must cite exact role-projection snapshot fact keys. Persist requested/resolved model, provider, token usage, cost, latency, finish reason and response hash, plus safe OpenRouter response-shape evidence (`error_type`, error/provider code, routing providers, generation ID, error-message hash); never persist raw model output, raw prompt, or raw error text. `UPSTREAM_ERROR`, `MISSING_CHOICE`, `EMPTY_CONTENT`, `OUTPUT_NOT_JSON`, and `SCHEMA_INVALID` are distinct failure classes and Telegram must humanize them without leaking internal codes.
...
- Off-hours manual decision triggers use the fixed 2026-07-13 14:45 ET `UAT_REPLAY`, clearly marked non-live, and make no model call. Telegram remains suppressed unless `?deliver` is explicit.
