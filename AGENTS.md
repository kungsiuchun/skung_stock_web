...
- Council agents use `google/gemma-4-26b-a4b-it` with strict JSON Schema, `temperature=0`, and `max_tokens=420`; they output analysis votes `CALL`, `PUT`, or `HOLD`, never `OPEN_*`. CIO uses `openai/gpt-5-mini` with strict JSON Schema and `max_completion_tokens=520`, but no `temperature`. Both use 12-second timeouts and at most one same-model retry for timeout/429/5xx/schema failure; there is no cross-model fallback.
