...
- Council agents use `google/gemma-4-26b-a4b-it` with strict JSON Schema, `temperature=0`, a compact v2 wire contract, and `max_tokens=512` (`640` only for one same-model retry after truncation). They output `CALL`, `PUT`, or `HOLD`, never `OPEN_*`. Requests rotate same-model providers in this order: DeepInfra -> Parasail -> NextBit -> Google Vertex; retry attempt two rotates the order once and ignores a provider named by failure metadata. Keep `require_parameters=true`, `allow_fallbacks=true`, a 45-second attempt timeout, a shared 100-second Council deadline, and an 8KB role-projection hard limit. CIO remains `openai/gpt-5-mini` with strict JSON Schema, `max_completion_tokens=520`, no `temperature`, and its existing timeout profile. There is no cross-model fallback.
